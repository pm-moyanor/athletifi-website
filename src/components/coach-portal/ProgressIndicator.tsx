  ...
  return (
    <div className="flex items-center justify-center lg:gap-40 md:gap-30 gap-4 py-5">
      {steps.map((step, index) => (
        <div key={step.id} className="flex flex-col items-center justify-center gap-2 w-9/12">
          <div
          ...
          <div className="text-primary text-center h-5 xs:h-fit md:text-nowrap text-wrap text-xs md:text-base">{step.label}</div>
        </div>
      ))}
    </div>
  );
